import React, { useState, useEffect, useRef, useCallback } from "react";

// ---- アイコン(簡易SVG) ----------------------------------------------------
function IconNavigation({ size = 16, color = "currentColor", className }) {
  return (
    <svg className={className} width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="3 11 22 2 13 21 11 13 3 11" />
    </svg>
  );
}
function IconAlert({ size = 16, color = "currentColor", className }) {
  return (
    <svg className={className} width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z" />
      <line x1="12" y1="9" x2="12" y2="13" />
      <line x1="12" y1="17" x2="12.01" y2="17" />
    </svg>
  );
}
function IconTrain({ size = 16, color = "currentColor", style }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={style}>
      <rect x="4" y="3" width="16" height="13" rx="3" />
      <circle cx="8.5" cy="12.5" r="1.2" fill={color} />
      <circle cx="15.5" cy="12.5" r="1.2" fill={color} />
      <path d="M7 20 5 22M17 20l2 2M9 16v3M15 16v3" />
    </svg>
  );
}
function IconX({ size = 16, color = "currentColor" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      <line x1="18" y1="6" x2="6" y2="18" />
      <line x1="6" y1="6" x2="18" y2="18" />
    </svg>
  );
}

// ---- 路線データ ------------------------------------------------------------
const LINES = {
  函館本線: "#3C6E52",
  千歳線: "#4A5FA5",
  室蘭本線: "#8B5E3C",
  宗谷本線: "#2E6E8E",
  富良野線: "#8E5FA5",
  石北本線: "#B5502E",
  根室本線: "#A54A6E",
  釧網本線: "#4C8E76",
  北海道新幹線: "#C9A227",
  カスタム: "#7A5C8E",
};

// 座標はおおよその駅舎位置。実際のGPS精度により誤差が出るため、
// 判定半径(STAMP_RADIUS_M)には余裕を持たせています。
// 複数路線が交わる駅は、旅程上メインとなる路線ひとつに寄せて分類しています。
const STATIONS = [
  { id: "sapporo", name: "札幌", line: "函館本線", lat: 43.0686, lng: 141.3508 },
  { id: "otaru", name: "小樽", line: "函館本線", lat: 43.1907, lng: 140.9947 },
  { id: "teine", name: "手稲", line: "函館本線", lat: 43.1201, lng: 141.2159 },
  { id: "ebetsu", name: "江別", line: "函館本線", lat: 43.1043, lng: 141.5433 },
  { id: "eniwa", name: "恵庭", line: "千歳線", lat: 42.9101, lng: 141.5766 },
  { id: "chitose", name: "千歳", line: "千歳線", lat: 42.8267, lng: 141.652 },
  { id: "shinchitose", name: "新千歳空港", line: "千歳線", lat: 42.7752, lng: 141.6832 },
  { id: "tomakomai", name: "苫小牧", line: "室蘭本線", lat: 42.6337, lng: 141.6041 },
  { id: "date-monbetsu", name: "伊達紋別", line: "室蘭本線", lat: 42.47, lng: 140.832 },
  { id: "higashi-muroran", name: "東室蘭", line: "室蘭本線", lat: 42.377, lng: 140.963 },
  { id: "muroran", name: "室蘭", line: "室蘭本線", lat: 42.3153, lng: 140.9736 },
  { id: "iwamizawa", name: "岩見沢", line: "函館本線", lat: 43.1962, lng: 141.7719 },
  { id: "takikawa", name: "滝川", line: "函館本線", lat: 43.5566, lng: 141.9084 },
  { id: "noboribetsu", name: "登別", line: "室蘭本線", lat: 42.4239, lng: 141.1072 },
  { id: "toya", name: "洞爺", line: "室蘭本線", lat: 42.5814, lng: 140.8367 },
  { id: "oshamambe", name: "長万部", line: "函館本線", lat: 42.5119, lng: 140.6459 },
  { id: "asahikawa", name: "旭川", line: "函館本線", lat: 43.7593, lng: 142.3563 },
  { id: "fukagawa", name: "深川", line: "函館本線", lat: 43.7222, lng: 142.0561 },
  { id: "biei", name: "美瑛", line: "富良野線", lat: 43.5942, lng: 142.4636 },
  { id: "kamikawa", name: "上川", line: "石北本線", lat: 43.8467, lng: 142.8163 },
  { id: "shibetsu", name: "士別", line: "宗谷本線", lat: 44.1858, lng: 142.3853 },
  { id: "nayoro", name: "名寄", line: "宗谷本線", lat: 44.3559, lng: 142.4611 },
  { id: "wakkanai", name: "稚内", line: "宗谷本線", lat: 45.4153, lng: 141.6733 },
  { id: "furano", name: "富良野", line: "富良野線", lat: 43.3428, lng: 142.3822 },
  { id: "obihiro", name: "帯広", line: "根室本線", lat: 42.9153, lng: 143.1956 },
  { id: "kushiro", name: "釧路", line: "根室本線", lat: 42.985, lng: 144.382 },
  { id: "shibecha", name: "標茶", line: "釧網本線", lat: 43.1339, lng: 144.4818 },
  { id: "mashu", name: "摩周", line: "釧網本線", lat: 43.5928, lng: 144.5992 },
  { id: "abashiri", name: "網走", line: "石北本線", lat: 44.0211, lng: 144.2738 },
  { id: "kitami", name: "北見", line: "石北本線", lat: 43.8039, lng: 143.889 },
  { id: "akkeshi", name: "厚岸", line: "根室本線", lat: 43.045, lng: 144.8517 },
  { id: "shinhakodatehokuto", name: "新函館北斗", line: "北海道新幹線", lat: 41.9061, lng: 140.6472 },
  { id: "goryokaku", name: "五稜郭", line: "函館本線", lat: 41.7975, lng: 140.7422 },
  { id: "onumakoen", name: "大沼公園", line: "函館本線", lat: 41.9765, lng: 140.6772 },
  { id: "mori", name: "森", line: "函館本線", lat: 42.1094, lng: 140.5992 },
  { id: "hakodate", name: "函館", line: "函館本線", lat: 41.7737, lng: 140.7261 },
  { id: "kikonai", name: "木古内", line: "北海道新幹線", lat: 41.6875, lng: 140.4267 },
];

const STAMP_RADIUS_M = 500; // この距離以内でスタンプ可能
const STORAGE_KEY = "hokkaido-stampbook:data";

// ---- ユーティリティ ---------------------------------------------------
function distanceMeters(lat1, lon1, lat2, lon2) {
  const R = 6371000;
  const toRad = (d) => (d * Math.PI) / 180;
  const dLat = toRad(lat2 - lat1);
  const dLon = toRad(lon2 - lon1);
  const a =
    Math.sin(dLat / 2) ** 2 +
    Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLon / 2) ** 2;
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}

function formatDistance(m) {
  if (m == null) return "—";
  if (m < 1000) return `${Math.round(m)}m`;
  return `${(m / 1000).toFixed(1)}km`;
}

function formatStampDate(iso) {
  const d = new Date(iso);
  return d.toLocaleString("ja-JP", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
    minute: "2-digit",
  });
}

function formatShortDate(iso) {
  const d = new Date(iso);
  const yy = String(d.getFullYear()).slice(2);
  const mm = String(d.getMonth() + 1).padStart(2, "0");
  const dd = String(d.getDate()).padStart(2, "0");
  return `${yy}.${mm}.${dd}`;
}

function hashRotation(id) {
  let h = 0;
  for (let i = 0; i < id.length; i++) {
    h = (h * 31 + id.charCodeAt(i)) % 1000;
  }
  return (h % 27) - 13; // -13 〜 13度
}

function polarPoint(r, angleDeg) {
  const a = ((angleDeg - 90) * Math.PI) / 180;
  return { x: 50 + r * Math.cos(a), y: 50 + r * Math.sin(a) };
}

// ---- 硬券スタンプ本体(SVG) ------------------------------------------
function StampMark({ uid, name, dateLabel, rotation }) {
  const arcId = `arc-${uid}`;
  const filterId = `ink-${uid}`;
  const ticks = Array.from({ length: 16 }, (_, i) => i * (360 / 16));

  return (
    <div className="stamp-mark-wrap" style={{ transform: `rotate(${rotation}deg)` }}>
      <svg viewBox="0 0 100 100" width="74" height="74">
        <defs>
          <filter id={filterId} x="-30%" y="-30%" width="160%" height="160%">
            <feTurbulence
              type="fractalNoise"
              baseFrequency="0.85"
              numOctaves="2"
              seed={Math.abs(rotation) + 3}
              result="noise"
            />
            <feDisplacementMap in="SourceGraphic" in2="noise" scale="2.6" />
          </filter>
          <path id={arcId} d="M14,54 A38,38 0 0 1 86,54" fill="none" />
        </defs>
        <g filter={`url(#${filterId})`} fill="none" stroke="#BC3B2E">
          <circle cx="50" cy="50" r="45" strokeWidth="2.4" />
          <circle cx="50" cy="50" r="37" strokeWidth="1" />
          {ticks.map((angle, i) => {
            const p1 = polarPoint(41, angle);
            const p2 = polarPoint(44.5, angle);
            return <line key={i} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} strokeWidth="1" />;
          })}
        </g>
        <text fill="#BC3B2E" fontSize="9.5" fontFamily="'Kaisei Tokumin', serif" filter={`url(#${filterId})`}>
          <textPath href={`#${arcId}`} startOffset="50%" textAnchor="middle">
            {name}
          </textPath>
        </text>
        <text
          x="50"
          y="55"
          textAnchor="middle"
          fontFamily="'Courier Prime', monospace"
          fontSize="8"
          fill="#BC3B2E"
          filter={`url(#${filterId})`}
        >
          下車済
        </text>
        <text
          x="50"
          y="67"
          textAnchor="middle"
          fontFamily="'Courier Prime', monospace"
          fontSize="5.5"
          fill="#BC3B2E"
          filter={`url(#${filterId})`}
        >
          {dateLabel}
        </text>
      </svg>
    </div>
  );
}

export default function StationStampBook() {
  const [stamps, setStamps] = useState({}); // { [stationId]: { stampedAt, lat, lng } }
  const [customStations, setCustomStations] = useState([]); // 自分で追加した駅・地点
  const [storageState, setStorageState] = useState("loading"); // loading | ready | error
  const [position, setPosition] = useState(null); // { lat, lng, accuracy }
  const [geoStatus, setGeoStatus] = useState("idle"); // idle | watching | denied | error | unsupported | timeout
  const [showDemo, setShowDemo] = useState(false);
  const [showAddForm, setShowAddForm] = useState(false);
  const [customName, setCustomName] = useState("");
  const [manualMode, setManualMode] = useState(false);
  const [manualLat, setManualLat] = useState("");
  const [manualLng, setManualLng] = useState("");
  const watchIdRef = useRef(null);
  const positionRef = useRef(null);

  // 保存データの読み込み(このアプリを開いた本人だけのデータとして保存)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const result = await window.storage.get(STORAGE_KEY, false);
        if (!cancelled) {
          const parsed = result ? JSON.parse(result.value) : { stamps: {}, customStations: [] };
          setStamps(parsed.stamps || {});
          setCustomStations(parsed.customStations || []);
          setStorageState("ready");
        }
      } catch (e) {
        // キー未作成時はエラーになるため、空データとして続行
        if (!cancelled) {
          setStamps({});
          setCustomStations([]);
          setStorageState("ready");
        }
      }
    })();
    return () => {
      cancelled = true;
    };
  }, []);

  // 位置情報の監視開始
  useEffect(() => {
    if (!("geolocation" in navigator)) {
      setGeoStatus("unsupported");
      return;
    }
    setGeoStatus("watching");
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        const next = {
          lat: pos.coords.latitude,
          lng: pos.coords.longitude,
          accuracy: pos.coords.accuracy,
        };
        positionRef.current = next;
        setPosition(next);
        setGeoStatus("watching");
      },
      (err) => {
        setGeoStatus(err.code === 1 ? "denied" : "error");
      },
      { enableHighAccuracy: true, maximumAge: 8000, timeout: 20000 }
    );
    const timeoutId = setTimeout(() => {
      if (!positionRef.current) {
        setGeoStatus((cur) => (cur === "watching" ? "timeout" : cur));
      }
    }, 12000);
    return () => {
      if (watchIdRef.current != null) {
        navigator.geolocation.clearWatch(watchIdRef.current);
      }
      clearTimeout(timeoutId);
    };
  }, []);

  const persist = useCallback(async (stampsObj, customArr) => {
    try {
      const result = await window.storage.set(
        STORAGE_KEY,
        JSON.stringify({ stamps: stampsObj, customStations: customArr }),
        false
      );
      if (!result) {
        console.error("保存に失敗しました");
      }
    } catch (e) {
      console.error("save failed", e);
    }
  }, []);

  const handleStamp = (station, distance) => {
    if (distance == null || distance > STAMP_RADIUS_M) return;
    const next = {
      ...stamps,
      [station.id]: {
        stampedAt: new Date().toISOString(),
        lat: position?.lat,
        lng: position?.lng,
      },
    };
    setStamps(next);
    persist(next, customStations);
  };

  const handleAddCustomStation = () => {
    if (!customName.trim()) return;
    let lat, lng;
    if (manualMode) {
      lat = parseFloat(manualLat);
      lng = parseFloat(manualLng);
      if (Number.isNaN(lat) || Number.isNaN(lng)) return;
    } else {
      if (!position) return;
      lat = position.lat;
      lng = position.lng;
    }
    const id = `custom-${Date.now()}`;
    const next = [
      ...customStations,
      {
        id,
        name: customName.trim(),
        line: "カスタム",
        lat,
        lng,
        isCustom: true,
      },
    ];
    setCustomStations(next);
    persist(stamps, next);
    setCustomName("");
    setManualLat("");
    setManualLng("");
    setShowAddForm(false);
  };

  const handleDeleteCustomStation = (id) => {
    if (!window.confirm("この地点を削除しますか?押したスタンプも消えます。")) return;
    const nextCustom = customStations.filter((c) => c.id !== id);
    const nextStamps = { ...stamps };
    delete nextStamps[id];
    setCustomStations(nextCustom);
    setStamps(nextStamps);
    persist(nextStamps, nextCustom);
  };

  const allStations = [...STATIONS, ...customStations];

  const enriched = allStations.map((s) => {
    const distance = position ? distanceMeters(position.lat, position.lng, s.lat, s.lng) : null;
    return { ...s, distance, stamp: stamps[s.id] || null };
  });

  const uncollected = enriched
    .filter((s) => !s.stamp)
    .sort((a, b) => {
      if (a.distance == null && b.distance == null) return 0;
      if (a.distance == null) return 1;
      if (b.distance == null) return -1;
      return a.distance - b.distance;
    });

  const collected = enriched
    .filter((s) => s.stamp)
    .sort((a, b) => new Date(b.stamp.stampedAt) - new Date(a.stamp.stampedAt));

  const collectedCount = collected.length;
  const total = allStations.length;

  return (
    <div className="page">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Shippori+Mincho:wght@500;800&family=Kaisei+Tokumin:wght@500;700&family=Courier+Prime&family=Zen+Kaku+Gothic+New:wght@400;500;700;900&display=swap');

        * { box-sizing: border-box; }
        .page {
          min-height: 100vh;
          background: #2A2118;
          background-image:
            radial-gradient(ellipse at top, rgba(255,255,255,0.04), transparent 60%);
          font-family: 'Zen Kaku Gothic New', sans-serif;
          color: #EDE3CC;
          padding: 20px 14px 48px;
        }
        .container { max-width: 480px; margin: 0 auto; }

        .header { margin-bottom: 16px; }
        .header .eyebrow {
          font-family: 'Courier Prime', monospace;
          font-size: 11px;
          letter-spacing: 0.2em;
          color: #B8923A;
          text-transform: uppercase;
        }
        .header h1 {
          font-family: 'Shippori Mincho', serif;
          font-weight: 800;
          font-size: 26px;
          margin: 4px 0 2px;
          color: #F4EBD8;
          letter-spacing: 0.06em;
          line-height: 1.3;
        }
        .header p {
          font-size: 12.5px;
          color: #B7A98A;
          margin: 0 0 10px;
        }
        .header-actions {
          display: flex;
          gap: 8px;
          flex-wrap: wrap;
        }
        .demo-toggle {
          font-family: 'Zen Kaku Gothic New', sans-serif;
          font-size: 11.5px;
          color: #F4EBD8;
          background: rgba(255,255,255,0.08);
          border: 1px solid rgba(255,255,255,0.18);
          border-radius: 20px;
          padding: 6px 12px;
        }
        .demo-toggle:active { transform: scale(0.97); }

        .add-form {
          margin-top: 10px;
          display: flex;
          gap: 6px;
          flex-wrap: wrap;
          align-items: center;
        }
        .add-input {
          flex: 1;
          min-width: 140px;
          background: rgba(255,255,255,0.08);
          border: 1px solid rgba(255,255,255,0.2);
          border-radius: 6px;
          padding: 8px 10px;
          color: #F4EBD8;
          font-family: 'Zen Kaku Gothic New', sans-serif;
          font-size: 13px;
        }
        .add-input::placeholder { color: #8C8069; }
        .add-confirm {
          font-family: 'Courier Prime', monospace;
          font-size: 11.5px;
          color: #fff;
          background: #4A5FA5;
          border: none;
          border-radius: 6px;
          padding: 8px 12px;
          white-space: nowrap;
        }
        .add-confirm:disabled { opacity: 0.4; }
        .add-hint { font-size: 11px; color: #8C8069; margin-top: 4px; width: 100%; }
        .manual-toggle {
          width: 100%;
          text-align: left;
          background: none;
          border: none;
          color: #B8923A;
          font-size: 11.5px;
          text-decoration: underline;
          padding: 4px 0 0;
        }
        .manual-fields {
          width: 100%;
          display: flex;
          gap: 6px;
          flex-wrap: wrap;
          margin-top: 6px;
        }
        .manual-input { min-width: 110px; flex: 1; }

        .demo-ticket { margin-bottom: 18px; }
        .demo-ribbon {
          position: absolute;
          top: -1px;
          left: -1px;
          background: #4A5FA5;
          color: #fff;
          font-family: 'Courier Prime', monospace;
          font-size: 9px;
          letter-spacing: 0.1em;
          padding: 2px 8px 2px 6px;
          border-bottom-right-radius: 8px;
          z-index: 2;
        }

        .progress-row {
          display: flex;
          align-items: center;
          gap: 10px;
          margin: 16px 0;
          font-family: 'Courier Prime', monospace;
        }
        .progress-track {
          flex: 1;
          height: 6px;
          background: rgba(255,255,255,0.08);
          border-radius: 3px;
          overflow: hidden;
        }
        .progress-fill {
          height: 100%;
          background: linear-gradient(90deg, #BC3B2E, #d9614f);
          transition: width 0.4s ease;
        }
        .progress-count {
          font-size: 13px;
          color: #F4EBD8;
          white-space: nowrap;
        }

        .geo-card {
          display: flex;
          align-items: center;
          gap: 10px;
          background: rgba(255,255,255,0.05);
          border: 1px solid rgba(255,255,255,0.1);
          border-radius: 10px;
          padding: 10px 12px;
          margin-bottom: 18px;
          font-size: 12.5px;
        }
        .geo-card .icon { flex-shrink: 0; }
        .geo-card.warn { border-color: rgba(188,59,46,0.5); background: rgba(188,59,46,0.12); }
        .geo-mono { font-family: 'Courier Prime', monospace; color: #C9BFA8; }

        .section-label {
          font-family: 'Courier Prime', monospace;
          font-size: 11px;
          letter-spacing: 0.15em;
          color: #8C8069;
          margin: 22px 0 10px 4px;
          text-transform: uppercase;
        }

        .ticket {
          position: relative;
          display: flex;
          align-items: stretch;
          background: #EDE3CC;
          color: #2E2A24;
          border-radius: 6px;
          margin-bottom: 12px;
          overflow: hidden;
          box-shadow: 0 3px 8px rgba(0,0,0,0.35);
        }
        .ticket--stamped { min-height: 82px; }
        .ticket-stub {
          width: 10px;
          flex-shrink: 0;
          background-image: repeating-linear-gradient(
            to bottom,
            #2E2A24 0, #2E2A24 6px,
            transparent 6px, transparent 12px
          );
          background-color: transparent;
          position: relative;
        }
        .ticket-stub::before,
        .ticket-stub::after {
          content: "";
          position: absolute;
          left: -7px;
          width: 14px; height: 14px;
          background: #2A2118;
          border-radius: 50%;
        }
        .ticket-stub::before { top: -7px; }
        .ticket-stub::after { bottom: -7px; }

        .ticket-body {
          flex: 1;
          padding: 12px 12px 12px 14px;
          display: flex;
          align-items: center;
          justify-content: space-between;
          gap: 10px;
        }
        .ticket-main { min-width: 0; }
        .line-chip {
          display: inline-flex;
          align-items: center;
          line-height: 1;
          font-size: 10px;
          font-family: 'Courier Prime', monospace;
          padding: 4px 8px 3px;
          border-radius: 20px;
          color: #fff;
          margin-bottom: 6px;
          letter-spacing: 0.02em;
          white-space: nowrap;
        }
        .station-name {
          font-family: 'Kaisei Tokumin', serif;
          font-weight: 700;
          font-size: 20px;
          line-height: 1.25;
        }
        .ticket-side {
          text-align: right;
          flex-shrink: 0;
        }
        .distance-readout {
          font-family: 'Courier Prime', monospace;
          font-size: 15px;
          color: #4A4438;
        }
        .in-range-tag {
          font-family: 'Courier Prime', monospace;
          font-size: 10.5px;
          color: #BC3B2E;
          font-weight: bold;
        }

        .stamp-btn {
          margin-top: 6px;
          font-family: 'Zen Kaku Gothic New', sans-serif;
          font-weight: 700;
          font-size: 12px;
          color: #fff;
          background: #BC3B2E;
          border: none;
          border-radius: 20px;
          padding: 6px 14px;
          animation: pulse 1.8s infinite;
        }
        .stamp-btn:active { transform: scale(0.96); }
        @keyframes pulse {
          0%, 100% { box-shadow: 0 0 0 0 rgba(188,59,46,0.5); }
          50% { box-shadow: 0 0 0 6px rgba(188,59,46,0); }
        }

        .ticket--stamped .ticket-body { opacity: 0.96; }
        .punch-hole {
          position: absolute;
          top: 8px;
          right: 8px;
          width: 16px;
          height: 16px;
          border-radius: 50%;
          background: #2A2118;
          box-shadow: inset 0 1px 3px rgba(0,0,0,0.6);
        }
        .stamp-mark-wrap {
          position: absolute;
          right: 6px;
          bottom: -2px;
          opacity: 0.88;
          mix-blend-mode: multiply;
          pointer-events: none;
        }

        .delete-btn {
          position: absolute;
          top: 6px;
          left: 6px;
          width: 20px;
          height: 20px;
          border-radius: 50%;
          background: rgba(0,0,0,0.08);
          border: none;
          color: #7A5C8E;
          display: flex;
          align-items: center;
          justify-content: center;
          z-index: 3;
        }
        .delete-btn:active { transform: scale(0.9); }

        .empty-note {
          font-size: 12.5px;
          color: #8C8069;
          padding: 10px 4px;
        }

        .footnote {
          margin-top: 28px;
          font-size: 11px;
          color: #6E6450;
          line-height: 1.6;
        }
      `}</style>

      <div className="container">
        <div className="header">
          <div className="eyebrow">Hokkaido Ekinori Project</div>
          <h1>途中下車 硬券帖</h1>
          <p>駅から{STAMP_RADIUS_M}m以内に近づくとスタンプが押せます。</p>
          <div className="header-actions">
            <button className="demo-toggle" onClick={() => setShowDemo((v) => !v)}>
              {showDemo ? "サンプルを閉じる" : "スタンプ後のサンプルを見る"}
            </button>
            <button
              className="demo-toggle"
              onClick={() => {
                setShowAddForm((v) => !v);
                setShowDemo(false);
              }}
            >
              {showAddForm ? "閉じる" : "＋ 自分の駅を追加"}
            </button>
          </div>
          {showAddForm && (
            <div className="add-form">
              <input
                className="add-input"
                type="text"
                placeholder="駅・地点の名前(例: 散歩きっぷ 第1停留所)"
                value={customName}
                onChange={(e) => setCustomName(e.target.value)}
              />
              {!manualMode && (
                <button
                  className="add-confirm"
                  disabled={!position || !customName.trim()}
                  onClick={handleAddCustomStation}
                >
                  現在地で登録
                </button>
              )}
              {!manualMode && !position && (
                <div className="add-hint">
                  現在地を取得中です。少し待ってから登録するか、下の「緯度経度を手動入力」をお試しください。
                </div>
              )}
              <button className="manual-toggle" onClick={() => setManualMode((v) => !v)}>
                {manualMode ? "現在地取得に戻す" : "緯度経度を手動入力する"}
              </button>
              {manualMode && (
                <div className="manual-fields">
                  <input
                    className="add-input manual-input"
                    type="text"
                    inputMode="decimal"
                    placeholder="緯度 例: 43.0686"
                    value={manualLat}
                    onChange={(e) => setManualLat(e.target.value)}
                  />
                  <input
                    className="add-input manual-input"
                    type="text"
                    inputMode="decimal"
                    placeholder="経度 例: 141.3508"
                    value={manualLng}
                    onChange={(e) => setManualLng(e.target.value)}
                  />
                  <button
                    className="add-confirm"
                    disabled={
                      !customName.trim() ||
                      Number.isNaN(parseFloat(manualLat)) ||
                      Number.isNaN(parseFloat(manualLng))
                    }
                    onClick={handleAddCustomStation}
                  >
                    この座標で登録
                  </button>
                  <div className="add-hint">
                    地図アプリで場所を長押しすると出てくる緯度経度をコピーして貼り付けられます。
                  </div>
                </div>
              )}
            </div>
          )}
        </div>

        {showDemo && (
          <div className="ticket ticket--stamped demo-ticket">
            <div className="demo-ribbon">PREVIEW</div>
            <div className="ticket-stub" />
            <div className="ticket-body">
              <div className="ticket-main">
                <span className="line-chip" style={{ background: LINES["カスタム"] }}>
                  サンプル
                </span>
                <div className="station-name">サンプル駅</div>
              </div>
              <div className="ticket-side">
                <div className="geo-mono" style={{ fontSize: 10.5 }}>
                  2026/08/03 14:12
                </div>
              </div>
            </div>
            <div className="punch-hole" />
            <StampMark uid="demo" name="サンプル駅" dateLabel="26.08.03" rotation={6} />
          </div>
        )}

        <div className="progress-row">
          <div className="progress-track">
            <div
              className="progress-fill"
              style={{ width: `${(collectedCount / total) * 100}%` }}
            />
          </div>
          <div className="progress-count">
            {collectedCount} / {total}
          </div>
        </div>

        {geoStatus === "watching" && (
          <div className="geo-card">
            <IconNavigation className="icon" size={16} color="#B8923A" />
            <span className="geo-mono">
              {position
                ? `現在地取得中 (精度 ±${Math.round(position.accuracy)}m)`
                : "現在地を取得しています…"}
            </span>
          </div>
        )}
        {geoStatus === "timeout" && (
          <div className="geo-card warn">
            <IconAlert className="icon" size={16} color="#BC3B2E" />
            <span>
              位置情報の取得に時間がかかっています。端末の位置情報の許可設定をご確認いただくか、「＋
              自分の駅を追加」内の緯度経度を手動入力からお試しください。
            </span>
          </div>
        )}
        {geoStatus === "denied" && (
          <div className="geo-card warn">
            <IconAlert className="icon" size={16} color="#BC3B2E" />
            <span>位置情報が許可されていません。ブラウザの設定から位置情報を許可してください。</span>
          </div>
        )}
        {(geoStatus === "error" || geoStatus === "unsupported") && (
          <div className="geo-card warn">
            <IconAlert className="icon" size={16} color="#BC3B2E" />
            <span>位置情報を取得できませんでした。屋外で電波の良い場所でお試しください。</span>
          </div>
        )}

        <div className="section-label">未収集・近い順</div>
        {uncollected.length === 0 && (
          <div className="empty-note">すべての駅を収集しました。</div>
        )}
        {uncollected.map((s) => {
          const inRange = s.distance != null && s.distance <= STAMP_RADIUS_M;
          return (
            <div className="ticket" key={s.id}>
              {s.isCustom && (
                <button className="delete-btn" onClick={() => handleDeleteCustomStation(s.id)}>
                  <IconX size={12} />
                </button>
              )}
              <div className="ticket-stub" />
              <div className="ticket-body">
                <div className="ticket-main">
                  <span
                    className="line-chip"
                    style={{ background: LINES[s.line] }}
                  >
                    {s.line}
                  </span>
                  <div className="station-name">{s.name}</div>
                </div>
                <div className="ticket-side">
                  <div className="distance-readout">{formatDistance(s.distance)}</div>
                  {inRange ? (
                    <>
                      <div className="in-range-tag">圏内</div>
                      <button
                        className="stamp-btn"
                        onClick={() => handleStamp(s, s.distance)}
                      >
                        スタンプを押す
                      </button>
                    </>
                  ) : (
                    <div className="in-range-tag" style={{ color: "#A79C82" }}>
                      —
                    </div>
                  )}
                </div>
              </div>
            </div>
          );
        })}

        <div className="section-label">収集済み</div>
        {collected.length === 0 && (
          <div className="empty-note">まだ収集した駅はありません。</div>
        )}
        {collected.map((s) => (
          <div className="ticket ticket--stamped" key={s.id}>
            {s.isCustom && (
              <button className="delete-btn" onClick={() => handleDeleteCustomStation(s.id)}>
                <IconX size={12} />
              </button>
            )}
            <div className="ticket-stub" />
            <div className="ticket-body">
              <div className="ticket-main">
                <span
                  className="line-chip"
                  style={{ background: LINES[s.line] }}
                >
                  {s.line}
                </span>
                <div className="station-name">{s.name}</div>
              </div>
              <div className="ticket-side">
                <div className="geo-mono" style={{ fontSize: 10.5 }}>
                  {formatStampDate(s.stamp.stampedAt)}
                </div>
              </div>
            </div>
            <div className="punch-hole" />
            <StampMark
              uid={s.id}
              name={s.name}
              dateLabel={formatShortDate(s.stamp.stampedAt)}
              rotation={hashRotation(s.id)}
            />
          </div>
        ))}

        <div className="footnote">
          <IconTrain size={12} style={{ verticalAlign: "-1px", marginRight: 4 }} />
          駅座標はおおよその位置です。スマホのGPS精度により、実際に駅へ到着してもすぐに反映されない場合があります。データはこの端末・このアカウントにのみ保存されます。
        </div>
      </div>
    </div>
  );
}
